#!/usr/bin/env python3
"""LoRA DPO training (Phase 8).

Not specified in the NBF paper — all hyperparameters except the
shared SFT defaults (lr=2e-4, epochs=3) are LLaMA-Factory defaults
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--dpo-data", required=True,
                   help="Path to LLaMA-Factory DPO JSONL (build with build_preference_data.py).")
...
CONFIG_PATHS = {
    "llama-3-8b-instruct": "src/guardbound/baselines/configs/llama3_8b_dpo.yaml",
    "phi-4":                "src/guardbound/baselines/configs/phi4_dpo.yaml",
}
HF_MODEL_IDS = {
...
def main() -> None:
    ...
    if not config_path.exists():
        print(f"ERROR: DPO config not found: {config_path}", file=sys.stderr)
        sys.exit(1)
    dpo_data = Path(args.dpo_data)
    if not dpo_data.exists():
        print(f"ERROR: DPO data not found: {dpo_data}", file=sys.stderr)
        sys.exit(1)
    output_dir = Path(args.output_dir or f"checkpoints/lora_dpo/{args.model}")
    ...
    manifest = build_manifest(
        ...
        chosen_provenance=(
            ...
            "data/processed/<safety_responses>.jsonl)."
        ),
        rejected_provenance=(
    ...
    print(
        ...
        f"output_dir={output_dir} train_file={dpo_data}"
    )
