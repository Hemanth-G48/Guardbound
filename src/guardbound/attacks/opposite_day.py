"""Full paper implementation of Opposite Day attack.
...
- Uses GPT-4 to dynamically generate attack questions
- Has backtracking logic when model refuses
- Two-phase approach: first ask for opposing viewpoint, then request original
"""
...
def generate_opposite_day_step(
    ...
    """
    system_prompt = OPPOSITE_DAY_SYSTEM_PROMPT.format(conversationObjective=goal)
    messages = [
...
class OppositeDayAttack(MultiTurnAttack):
    """Opposite Day attack using GPT-4 dynamic generation.

    The attack works by:
    1. First asking for the opposing viewpoint (harmless)
    2. Then requesting the original harmful viewpoint
    3. Uses GPT-4 to generate and adapt questions dynamically
    4. Implements backtracking when model refuses
    """
    ...
    def __init__(
        ...
        """Initialize the Opposite Day attack.
    ...
    def set_safety_filter(self, embed_fn, barrier, threshold: float) -> None:
        ...
        When set, ``next_query`` applies the official Opposite Day candidate
        filtering (3 trials) before returning a prompt: a prompt whose NBF
        score is >= ``threshold`` is filtered and regenerated. Callers must
        pass ``threshold`` with the official sign convention
        (``threshold = -args.threshold`` in ``run_opposite_day``).
        """
    ...
    def generate_question_for_turn(
        ...
        return generate_opposite_day_step(
            round_num=turn_num,
            goal=goal,
    ...
    def record_turn(self, attacker_query: str, target_response: str, score: int) -> None:
        ...
        previous response (only for rounds > 1), so subsequent attacker
        prompts are built from summaries rather than raw responses.
        """
    ...
    def _dialog_hist_from_turns(self, history: list[Turn]) -> list[dict]:
        ...
        Filtered prompts are never sent to the target, so they are excluded,
        matching the official ``history_t``.
        """
        return [
    ...
    def _filter_prompt_with_nbf(self, prompt: str, history: list[Turn]) -> str:
        ...
        Mirrors ``run_opposite_day``: the author's loop evaluates the generated
        prompt once (the ``else`` branch also breaks), and on ``unsafe_cutoff``
        the prompt is filtered and the attack regenerates a new prompt.
        """
        ...
        while attempts < self._max_turns:
            ...
            # Filtered: regenerate a fresh prompt (official ``continue`` in the
            # while loop, with the round number offset by the filtering count).
            attempts += 1
